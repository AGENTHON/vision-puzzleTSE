# -*- coding: utf-8 -*-
"""
Created on Mon Jan 25 14:31:11 2021
...
def segment_piece(image, bin_threshold=128):
    ...
    """
    return cv2.threshold(image, bin_threshold, 255, cv2.THRESH_BINARY_INV)[1]
...
def shape_classification(edges, line_params, d_threshold=500, n_hs=10):
    ...
    for x_edge, y_edge in zip(x_nonzero, y_nonzero):
        d = [distance_point_line_squared(a,b,c, x_edge, y_edge) for (a,b,c) in line_params]
...
def compute_inout(class_image, line_params, xb, yb, d_threshold=10):
    ...
    for line_param, cl in zip(line_params, (1, 2, 3, 4)):
        ...
        distances = np.array([distance_point_line_signed(a,b,c, x0, y0) for y0, x0 in coords])    
        distances = distances[np.abs(distances) > d_threshold]
        m_dist = np.mean(distances)

        b_dist = distance_point_line_signed(a,b,c, xb, yb)
...
def get_best_fitting_rect_coords(xy, d_threshold=30, perp_angle_thresh=20, verbose=0):
    ...
    def search_for_possible_rectangle(idx, prev_points=[]):
        ...
        if depth in (1, 2):
            ...
            if verbose >= 2:
                print('\t' * depth, 'diff0:', np.nonzero(diff0)[0], 'diff180_0:', np.nonzero(diff180_0)[0], 'diff_to_explore:', diff_to_explore)
...
def process_piece(image, **kwargs):
    ...
    try:
        ...
        out_dict['gray_harris'] = gray_harris
...
postprocess = partial(cv2.blur, ksize=(5, 5))
results = []
error_labels = []

img_names=["piece","coin"]
for name in img_names:

    img = cv2.imread('./Banque/snow-white/'+name+'.jpg')
